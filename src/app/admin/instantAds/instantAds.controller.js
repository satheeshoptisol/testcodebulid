export class InstantAdsController {
  constructor(toaster, $uibModal, ManageAdsService, $stateParams, $state, $scope, $auth, $log, $window, $q, AWS_CONFIG) {
    'ngInject';

    // this.myClass = { createAdvertisementForm: $stateParams.page == 'create' };
    // this.editAdClass = { editAdvertisementForm: $stateParams.page == 'edit', id: $stateParams.id };
    // this.viewAdClass = { viewAdvertisement: $stateParams.page == 'view', id: $stateParams.id };
    ...
    this.countryList = angular.copy(this.$scope.manageAd.countryTypes);
    this.datePicker = {};
    ...
    this.dateOptions1 = {
      ...
      minDate: Date.now(),
      formatYear: 'yyyy',
      // minDate: this.datePicker.minStartDate,
      startingDay: 1
    };
    ...
    if ($stateParams.page == 'edit') {
      this.toggleEditAdClass($stateParams.id, 'instantAds');
    }
    ...
    if ($stateParams.page == 'view') {
      this.toggleViewAdClass($stateParams.id, 'instantAds');
    }
  ...
  addMoreFieldSet() {
    var newDepotNo = this.choices.length + 1;
    this.choices.push({ 'id': newDepotNo })
  }
  ...
  removeChoice(index) {
    ...
    if (lastItem === 0) {
      ...
      if (this.editAds) {
        _.map(this.editAds.flipCountsToWin, (obj, key) => {
          if (index == key) {
            delete this.editAds.flipCountsToWin[key];
          }
        })
      }
  ...
  toggleEditAdClass(id, type) {
    ...
    this.ManageAdsService.getAdvertisement(id, type).then(response => {
      ...
      angular.forEach(this.editAds.flipCountsToWin, (value, key) => {
        this.choices.push(key);
        this.editAds.flipCountsToWin[key].showFlipName = true;
      });
      ...
      _.map(this.editAds.advertiserList, advertiser => {
        if (_.includes(advertiser.createdBy, this.editAds.createdBy)) {
          advertiser.ticked = true;
        }
        else {
          advertiser.ticked = false;
        }
      })
      ...
      if (this.editAds.os.length >= 1) {
        ...
        this.os = true;
        this.editAds.osTypes = angular.copy(this.osTypes);
      ...
      if (this.editAds.gender.length >= 1) {
        ...
        this.editAds.genderTypes = angular.copy(this.genderTypes);
      ...
      if (this.editAds.countries.length >= 1) {
        ...
        this.editAds.countryTypes = angular.copy(this.$scope.manageAd.countryTypes);
  ...
  instantCountry(value) {
    if (value) {
      ...
      this.countryModel = '';
      if (this.editAds) {
        this.editAds.countryModel = '';
      }
    }
  }

  // instant User Segment toggle
  instantUserSegment(value) {
    if (value) {
      this.showUserSegmentDropDown = true;
    }
    else {
      this.showUserSegmentDropDown = false;
    }
  }
  ...
  postAdvertisement(form, type) {
    ...
    // if((this.ads.flipCountsToWin == undefined) || (this.ads.flipCountsToWin.length == 0)) {
    //   this.flipCountMessage = "Required";
    // }
    ...
    if ((form.$valid && this.errMessage == '' && this.startErrMessage == '' && !this.mediaErrorMessage) && (!this.osError && !this.genderError && !this.countryError && !this.zipCodeError)) {
        var newArr = _.map(this.ads.flipCountsToWin, function(o) {
        return _.pick(o, 'count', 'prize', 'media'); 
      });

      var newObj = {};
      var newObject = angular.extend(newObj,newArr);
      ...
      if (this.$auth.getPayload().user.role.name === 'admin') {
        this.ManageAdsService.createAdvertisement(data, type).then(response => {
          this.ManageAdsService.updateAd(response.id, type).then(updatedResponse => {
            if (updatedResponse && this.getAdvertiserListModel.length > 0) {
              let adData = {
                adType: 'InstantAd',
                adId: response.id,
                assignTo: this.getAdvertiserListModel[0].id
            ...
            this.$scope.manageAd.adTypesClass.adTypesForm = false;
            this.$state.go('manage-ads');
            this.$scope.manageAd.filteredList.push(updatedResponse);
            this.$scope.manageAd.setPagingData(this.currentPage);
        ...
        this.ManageAdsService.createAdvertisement(data, type).then(response => {
          ...
          if (!response.paid) {
            this.$state.go('payment', { adId: response.id, adType: 'instantAd' });
          }
          else {
  ...
  publishEditedAdvertisement(form) {
    ...
    if (form.$valid && (this.errMessage == '' || this.errMessage == undefined) && (this.startErrMessage == '' || this.startErrMessage == undefined) && (!this.osError && !this.genderError && !this.countryError && !this.zipCodeError) && this.editAds.media.length > 0 && !this.maxSizeError) {
      let data = {
        ...
        flipCountsToWin: this.editAds.flipCountsToWin,
        gender: gender,
        countries: country,
      ...
      if (this.$auth.getPayload().user.role.name === 'admin') {
        this.ManageAdsService.editAdvertisement(this.editAds.id, 'instantAds', data).then(response => {
        ...
        this.ManageAdsService.editAdvertisement(this.editAds.id, 'instantAds', editData).then(response => {
          let index = _.findIndex(this.$scope.manageAd.filteredList, (obj) => obj.id == response.id)
          this.$scope.manageAd.filteredList[index] = response;
          this.$scope.manageAd.setPagingData(this.$scope.manageAd.currentPage);
          this.editAdClass.editAdvertisementForm = false;
  ...
  editAdvertisement(form) {
    ...
    this.$log.log(this.editAds.media)
    let osTypeList = [];
    let gender = [];
    let country = [];
    let zipCodes = [];
  ...
  uploadFiles(file) {
    ...
    if (file.length == 0) {
      ...
      this.uploadFile(file[0]).then(uploadedFile => {
        ...
        let uploadedFileName = uploadedFile.Location.split("/").pop();
        this.media = [];
        this.media = [{
          url: `${this.AWS_CONFIG.CLOUDFRONT_ORG_URL}/${uploadedFileName}`,
          thumbnail: `${this.AWS_CONFIG.CLOUDFRONT_URL}/thumbnail/${uploadedFileName}`,
          large: `${this.AWS_CONFIG.CLOUDFRONT_URL}/large/${uploadedFileName}`,
          small: `${this.AWS_CONFIG.CLOUDFRONT_URL}/small/${uploadedFileName}`,
          name: file[0].name,
          type: 'image'
        }]
        ...
        if (this.editAds && this.editAds.media) {
          this.ManageAdsService.isImage(this.editAds.media).then(result => {
            if (result) {
              this.editAds.media = [{
                width: window.width,
        ...
        if (this.media) {
          this.ManageAdsService.isImage(this.media).then(result => {
            if (result) {
              this.showImage = true;
              this.media = [{
                width: window.width,
  ...
  // file upload
  uploadFlipCountFiles(file, $index) {
    if(this.ads) {
      if (file.length == 0) {
        this.ads.flipCountsToWin[$index].maxSizeError = true;
        this.ads.flipCountsToWin[$index].flipErrorMessage = true;
        this.ads.flipCountsToWin[$index].showFlipName = false;
        this.ads.flipCountsToWin[$index].showFlipMedia = false;
      }
      else {
        this.maxSizeError = false;
        this.ads.flipCountsToWin[$index].progressFlipMessage = true;
        this.ads.flipCountsToWin[$index].flipErrorMessage = false;
        this.ads.flipCountsToWin[$index].editFlipErrorMsg = false;
        this.uploadFile(file[0]).then(uploadedFile => {
        var img = new Image();
        img.onload = function () {
          window.flipWidth = this.width;
          window.flipHeight = this.height;
        };
          // Mark as success
        let uploadedFileName = uploadedFile.Location.split("/").pop();
          this.$log.log("After upload flip image")
          this.$log.log(uploadedFile)
          let width = (parseFloat(window.flipWidth)/parseFloat(window.flipWidth) );
          let height = (parseFloat(window.flipHeight)/parseFloat(window.flipWidth)).toFixed(2);
          this.ads.flipCountsToWin[$index].media = {};
          this.ads.flipCountsToWin[$index].media = {
            width: window.flipWidth,
            height: window.flipHeight,
            url: `${this.AWS_CONFIG.CLOUDFRONT_ORG_URL}/${uploadedFileName}`,
            thumbnail: `${this.AWS_CONFIG.CLOUDFRONT_URL}/thumbnail/${uploadedFileName}`,
            large: `${this.AWS_CONFIG.CLOUDFRONT_URL}/large/${uploadedFileName}`,
            small: `${this.AWS_CONFIG.CLOUDFRONT_URL}/small/${uploadedFileName}`,
          ...
          img.src = uploadedFile.Location;
          this.ads.flipCountsToWin[$index].flipName = file[0].name;
        }, error => {
          // Mark the error
          this.Error = error;
        }, progress => {
          // Write the progress as a percentage
          this.progressObj = progress;
          this.ads.flipCountsToWin[$index].Progress = (progress.loaded / progress.total) * 100
          if (this.ads.flipCountsToWin[$index].Progress === 100) {
            this.ads.flipCountsToWin[$index].showFlipName = true;
            this.ads.flipCountsToWin[$index].showFlipMedia = true;
            // this.showRemoveMedia = true;
            this.ads.flipCountsToWin[$index].maxSizeError = false;
            this.ads.flipCountsToWin[$index].flipErrorMessage = false;
            this.ads.flipCountsToWin[$index].editFlipErrorMsg = false;
            this.ads.flipCountsToWin[$index].progressFlipMessage = false;
          }
        });
      }
    }
    if(this.editAds) {
      if (file.length == 0) {
      this.editAds.flipCountsToWin[$index].maxSizeError = true;
      this.editAds.flipCountsToWin[$index].flipErrorMessage = true;
      this.editAds.flipCountsToWin[$index].editFlipErrorMsg = false;
      this.editAds.flipCountsToWin[$index].showFlipName = false;
      this.editAds.flipCountsToWin[$index].showFlipMedia = false;
    }
    else {
      this.maxSizeError = false;
      this.editAds.flipCountsToWin[$index].progressFlipMessage = true;
      this.editAds.flipCountsToWin[$index].flipErrorMessage = false;
      this.editAds.flipCountsToWin[$index].editFlipErrorMsg = false;
      this.uploadFile(file[0]).then(uploadedFile => {
        // Mark as success
          var img = new Image();
        ...
        this.$log.log("After upload edit flip image")
        this.$log.log(uploadedFile)
        let uploadedFileName = uploadedFile.Location.split("/").pop();
        img.src = uploadedFile.Location;
        let width = (parseFloat(window.flipWidth)/parseFloat(window.flipWidth) );
        let height = (parseFloat(window.flipHeight)/parseFloat(window.flipWidth)).toFixed(2);
        this.editAds.flipCountsToWin[$index].media = {};
        this.editAds.flipCountsToWin[$index].media = {
          width: window.flipWidth,
          height: window.flipHeight,
          url: `${this.AWS_CONFIG.CLOUDFRONT_ORG_URL}/${uploadedFileName}`,
        ...
        this.editAds.flipCountsToWin[$index].flipName = file[0].name;
      }, error => {
        // Mark the error
        this.Error = error;
      }, progress => {
        // Write the progress as a percentage
        this.progressObj = progress;
        this.editAds.flipCountsToWin[$index].Progress = (progress.loaded / progress.total) * 100
        if (this.editAds.flipCountsToWin[$index].Progress === 100) {
          this.editAds.flipCountsToWin[$index].showFlipName = true;
          this.editAds.flipCountsToWin[$index].showFlipMedia = true;
          // this.showRemoveMedia = true;
          this.editAds.flipCountsToWin[$index].maxSizeError = false;
          this.editAds.flipCountsToWin[$index].flipErrorMessage = false;
          this.editAds.flipCountsToWin[$index].editFlipErrorMsg = false;
          this.editAds.flipCountsToWin[$index].progressFlipMessage = false;
        }
      });
    }
    }
  ...
  deleteFlipFile($index) {
    this.$log.log("Delete file")
    // this.uploader.abort();
    if (this.ads && this.ads.flipCountsToWin[$index]) {
      this.ads.flipCountsToWin[$index].showFlipName = false;
      this.ads.flipCountsToWin[$index].showFlipMedia = false;
      this.ads.flipCountsToWin[$index].maxSizeError = false;
      this.ads.flipCountsToWin[$index].flipErrorMessage = false;
      this.ads.flipCountsToWin[$index].Progress = 0;
      this.ads.flipCountsToWin[$index].progressFlipMessage = false;
    }
    if (this.editAds && this.editAds.flipCountsToWin[$index]) {
      this.editAds.flipCountsToWin[$index].showFlipName = false;
      this.editAds.flipCountsToWin[$index].showFlipMedia = false;
      this.editAds.flipCountsToWin[$index].maxSizeError = false;
      this.editAds.flipCountsToWin[$index].flipErrorMessage = false;
      this.editAds.flipCountsToWin[$index].Progress = 0;
      this.editAds.flipCountsToWin[$index].progressFlipMessage = false;
    }
  }
